...
async function saveAttachment(
  ...
  try {
    Zotero.debug(
      `ZotBag: Saving ${contentType} attachment for item: ${item.getField("title")}`,
    );
    ...
    Zotero.debug(
      `ZotBag: Error saving ${contentType} attachment: ${error.message}`,
    );
    throw error;
  }
}
...
export async function createZoteroItemFromWallabagEntry(
  ...
  try {
    Zotero.debug(
      `ZotBag: Creating Zotero item for Wallabag entry: ${entry.title}`,
    );
    ...
    try {
      ...
      if (collection) {
        Zotero.debug(
          `ZotBag: Adding item ${item.id} to Wallabag collection ${collection.id}`,
        );
        ...
        await item.saveTx();
        Zotero.debug(
          `ZotBag: Added item to collection using item.addToCollection() and saved item`,
        );
      } else {
      ...
      Zotero.debug(
        `ZotBag: Error adding item to Wallabag collection: ${error.message}`,
      );
        ...
        for (const { format, pref, contentType } of formats) {
          if (getPref(pref as any)) {
            try {
              ...
              const data = await wallabagApi.downloadEntryInFormat(
                entry.id,
                format,
              );
    ...
    Zotero.debug(
      `ZotBag: Successfully created Zotero item for Wallabag entry: ${entry.title}`,
    );
    return item;
  } catch (error: any) {
  ...
  try {
    ...
    for (const collection of collections) {
      if (collection.name === "Wallabag") {
        Zotero.debug(
          `ZotBag: Found existing Wallabag collection with ID ${collection.id}`,
        );
        return collection;
      }
    }
    ...
    Zotero.debug(
      `ZotBag: Created new Wallabag collection with ID ${collection.id}`,
    );
    return collection;
  } catch (error: any) {
    Zotero.debug(
      `ZotBag: Error getting or creating Wallabag collection: ${error.message}`,
    );
    return null;
  }
}
