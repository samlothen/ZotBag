...
export class WallabagAPI {
  ...
  private async getAccessToken(): Promise<string> {
    try {
      ...
      if (!response.ok) {
        ...
        Zotero.debug(
          `ZotBag: Failed to get access token. Status: ${response.status}, Response: ${errorText}`,
        );
        throw new Error(
  ...
  async getEntry(entryId: number): Promise<WallabagEntry> {
    try {
      ...
      if (
        ...
        Zotero.debug(
          "ZotBag: Missing required credentials for Wallabag connection",
        );
        throw new Error(
          "Please fill in all Wallabag credentials in the settings",
        );
      }
      ...
      if (!response.ok) {
        const errorText = await response.text();
        Zotero.debug(
          `ZotBag: Failed to fetch entry. Status: ${response.status}, Response: ${errorText}`,
        );
        throw new Error(
   ...
   */
  async downloadEntryInFormat(
    entryId: number,
    format: string,
  ): Promise<ArrayBuffer> {
    try {
      Zotero.debug(
        `ZotBag: Downloading ${format.toUpperCase()} for entry with ID ${entryId}`,
      );
      ...
      if (!response.ok) {
        const errorText = await response.text();
        Zotero.debug(
          `ZotBag: Failed to download ${format.toUpperCase()}. Status: ${response.status}, Response: ${errorText}`,
        );
        throw new Error(
          `Failed to download ${format.toUpperCase()}: ${response.status} ${response.statusText}`,
        );
      }
      ...
      Zotero.debug(
        `ZotBag: Error downloading ${format.toUpperCase()}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getEntries(
    ...
    try {
      Zotero.debug(
        `ZotBag: Fetching entries page ${page} (since: ${since || "all"})`,
      );
      ...
      if (!response.ok) {
        const errorText = await response.text();
        Zotero.debug(
          `ZotBag: Failed to fetch entries. Status: ${response.status}, Response: ${errorText}`,
        );
        throw new Error(
      ...
      Zotero.debug(
        `ZotBag: Successfully fetched ${entriesResponse._embedded.items.length} entries (page ${entriesResponse.page} of ${entriesResponse.pages})`,
      );
  ...
  async getAllEntries(
    ...
    try {
      ...
      Zotero.debug(
        `ZotBag: Successfully fetched all ${allEntries.length} entries`,
      );
      return allEntries;
    } catch (error: any) {
  ...
  async testConnection(): Promise<{
    ...
    try {
      ...
      if (!infoResponse.ok) {
        ...
        Zotero.debug(
          `ZotBag: Failed to get server info. Status: ${infoResponse.status}, Response: ${errorText}`,
        );
        return {
          success: false,
      ...
      const serverInfo = info as unknown as {
        appname: string;
        version: string;
        allowed_registration: boolean;
      };
      Zotero.debug(
        `ZotBag: Successfully connected to Wallabag server. Version: ${serverInfo.version}`,
      );
