...
export class WallabagSync {
  ...
  startSync(): void {
    ...
    // Schedule the sync
    Zotero.debug(
      `ZotBag: Starting sync timer with interval of ${syncIntervalMinutes} minutes`,
    );
    this.syncTimer = Zotero.getMainWindow().setInterval(() => {
      this.syncWallabagEntries().catch((error) => {
  ...
  stopSync(): void {
    if (this.syncTimer !== null) {
      ...
      Zotero.getMainWindow().clearInterval(this.syncTimer);
      this.syncTimer = null;
    }
  }
   ...
   */
  async syncWallabagEntries(
    showProgress: boolean = true,
  ): Promise<{ added: number; updated: number; errors: number } | null> {
    ...
    try {
      ...
      const lastSyncTimestamp = getPref("wallabag.sync.lastTimestamp");
      ...
      Zotero.debug(
        `ZotBag: Sync complete: ${result.added} added, ${result.updated} updated, ${result.errors} errors`,
      );
      return result;
    } catch (error: any) {
  ...
  private async processEntries(
    ...
    for (let i = 0; i < entries.length; i++) {
      try {
        ...
        const existingItems = await this.findExistingZoteroItems(
          entry.id,
          libraryID,
        );
   ...
   */
  private async findExistingZoteroItems(
    entryId: number,
    libraryID: number,
  ): Promise<Zotero.Item[]> {
   ...
   */
  private async updateZoteroItem(
    item: Zotero.Item,
    entry: WallabagEntry,
    downloadPdf: boolean,
  ): Promise<void> {
    try {
      Zotero.debug(
        `ZotBag: Updating Zotero item for Wallabag entry: ${entry.title}`,
      );
      ...
      try {
        ...
        if (downloadPdf) {
          ...
          if (!hasPdf) {
            try {
              // Download the PDF
              const pdfData = await this.wallabagApi.downloadEntryAsPdf(
                entry.id,
              );
              ...
              const fileOutputStream = (Components.classes as any)[
              ...
              // Write the data
              const binaryOutputStream = (Components.classes as any)[
            ...
            if (getPref(pref as any)) {
              try {
                ...
                const hasFormat = attachments.some((attachment) => {
                  const attachmentFilename = attachment.getField(
                    "title",
                  ) as string;
                ...
                if (!hasFormat) {
                  Zotero.debug(
                    `ZotBag: Downloading ${format} for entry ${entry.id}`,
                  );
                  const data = await this.wallabagApi.downloadEntryInFormat(
                    entry.id,
                    format,
                  );
                ...
                Zotero.debug(
                  `ZotBag: Error attaching ${format.toUpperCase()}: ${formatError.message}`,
                );
      ...
      Zotero.debug(
        `ZotBag: Successfully updated Zotero item for Wallabag entry: ${entry.title}`,
      );
    } catch (error: any) {
